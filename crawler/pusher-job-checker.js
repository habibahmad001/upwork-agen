/**
 * Pusher Job Checker - For /pusher-test page
 *
 * Features:
 * - Crawls Upwork every 60 seconds
 * - Evaluates jobs with Groq AI (basic)
 * - Sends Pusher notifications
 *
 * Usage: node pusher-job-checker.js
 */
...
const KNOWN_JOBS_PATH = resolve('./known-jobs-pusher.json');
const OUTPUT_PATH = resolve('./jobs-pusher.json');
...
console.log('🚀 Pusher Job Checker (for /pusher-test)');
console.log('==========================================\n');
...
/**
 * Evaluate job with Groq AI (simplified for pusher-test)
 */
async function evaluateJobWithAI(job) {
  const prompt = `You are an expert freelance job evaluator. Quickly analyze this Upwork job.
...
Provide a score (0-100) and brief recommendation.
  ...
  try {
    for (const job of jobsWithAI) {
      const notificationData = {
        ...
        recommendation: job.recommendation || 'consider',
...
async function start() {
  ...
  if (autoStart) {
    ...
    console.log('✅ Starting Pusher job checker automatically...\n');
  } else {
    console.log('\n⏸️  PAUSED:');
