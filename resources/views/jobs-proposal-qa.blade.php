...
<head>
    ...
    <title>Jobs with Q&A Proposal</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .add-qa-btn:hover { background: #15803d; }
        .qa-inputs {
            display: flex;
        ...
        .remove-qa:hover { background: #dc2626; }
        ...
        .proposal-btn:hover { transform: translateY(-2px); box-shadow: 0 4px 12px rgba(139, 92, 246, 0.4); }
        .proposal-btn:disabled { background: #9ca3af; cursor: not-allowed; }
...
<body>
    ...
    <div id="proposalModal" class="proposal-modal">
        <div class="modal-content">
            <div class="modal-header">
                <span class="modal-title">📝 Proposal + Q&A</span>
            ...
            <div class="modal-footer">
                <button class="copy-btn" onclick="copyProposal()" id="copyBtn">📋 Copy All</button>
            </div>
        </div>
    </div>
    ...
    <script>
        const CONFIG = {
            ...
            qaApiUrl: '/api/proposal/generate-qa',
            pusher: {
            ...
            try {
                ...
                channel.bind_global((eventName, data) => {
                    if (eventName === CONFIG.pusher.event && data.type !== 'test') {
                        ...
                        showDesktopNotification(emoji + ' ' + (data.title || 'New Job'), `AI Score: ${data.ai_score}/100 - ${data.recommendation}`);
                    }
                });
        ...
        function createJobCard(job) {
            ...
            card.innerHTML = `
                ...
                <div class="qa-section">
                    <div class="qa-header">
                        <span class="qa-title">❓ Your Questions for Client</span>
                        <button class="add-qa-btn" onclick="addQuestion('${jobCounter}')">+ Add Question</button>
                    </div>
                    <div class="qa-inputs" id="qa-inputs-${jobCounter}">
                        <div class="qa-input-group">
                            <span class="qa-input-number">1</span>
                            <input type="text" class="qa-input" placeholder="What's your main concern about this project?" id="qa-${jobCounter}-0" onchange="saveQuestion('${jobCounter}', 0, this.value)">
                            <button class="remove-qa" onclick="removeQuestion('${jobCounter}', 0)" style="visibility: hidden;">×</button>
                        </div>
                        <div class="qa-input-group">
                            <span class="qa-input-number">2</span>
                            <input type="text" class="qa-input" placeholder="What's the expected timeline for completion?" id="qa-${jobCounter}-1" onchange="saveQuestion('${jobCounter}', 1, this.value)">
                            <button class="remove-qa" onclick="removeQuestion('${jobCounter}', 1)" style="visibility: hidden;">×</button>
                        </div>
                        <div class="qa-input-group">
                            <span class="qa-input-number">3</span>
                            <input type="text" class="qa-input" placeholder="Do you have any existing assets or materials?" id="qa-${jobCounter}-2" onchange="saveQuestion('${jobCounter}', 2, this.value)">
                            <button class="remove-qa" onclick="removeQuestion('${jobCounter}', 2)" style="visibility: hidden;">×</button>
                        </div>
                    </div>
                </div>
            ...
            // Initialize questions for this job
            jobQuestions[jobCounter] = [
        ...
        function addQuestion(jobCounter) {
            const container = document.getElementById('qa-inputs-' + jobCounter);
            const questionIndex = jobQuestions[jobCounter].length;

            const div = document.createElement('div');
            div.className = 'qa-input-group';
            div.innerHTML = `
                <span class="qa-input-number">${questionIndex + 1}</span>
                <input type="text" class="qa-input" placeholder="Enter your question..." id="qa-${jobCounter}-${questionIndex}" onchange="saveQuestion('${jobCounter}', ${questionIndex}, this.value)">
                <button class="remove-qa" onclick="removeQuestion('${jobCounter}', ${questionIndex})">×</button>
            `;

            container.appendChild(div);
            jobQuestions[jobCounter].push({ question: '', answer: null });
        }

        function removeQuestion(jobCounter, questionIndex) {
            const container = document.getElementById('qa-inputs-' + jobCounter);
            const inputs = container.querySelectorAll('.qa-input-group');

            if (inputs.length > 1) {
                container.removeChild(inputs[questionIndex]);
                jobQuestions[jobCounter].splice(questionIndex, 1);

                // Renumber remaining questions
                container.querySelectorAll('.qa-input-group').forEach((group, index) => {
                    group.querySelector('.qa-input-number').textContent = index + 1;
                    const input = group.querySelector('.qa-input');
                    input.id = `qa-${jobCounter}-${index}`;
                    input.setAttribute('onchange', `saveQuestion('${jobCounter}', ${index}, this.value)`);
                    const removeBtn = group.querySelector('.remove-qa');
                    removeBtn.setAttribute('onclick', `removeQuestion('${jobCounter}', ${index})`);
                });
            }
        }

        function saveQuestion(jobCounter, questionIndex, value) {
            if (jobQuestions[jobCounter]) {
                jobQuestions[jobCounter][questionIndex].question = value;
            }
        }

        // Proposal generation with Q&A
        let currentProposal = '';
        let currentJobData = {};
        ...
        async function generateProposal(btn, jobData) {
            ...
            try {
                const result = await generateQAProposal();
                currentProposal = result.fullProposal;

                // Display proposal with Q&A
                const modalContent = document.getElementById('proposalContent');
                modalContent.innerHTML = result.formattedProposal;
            ...
            btn.disabled = false;
            btn.innerHTML = '📝 Generate Proposal + Q&A';
        }

        async function generateQAProposal() {
            const questions = jobQuestions[jobCounter].filter(q => q.question && q.question.trim()).map(q => q.question);

            const response = await fetch(CONFIG.qaApiUrl, {
                method: 'POST',
                headers: {
            ...
            // Format the output with Q&A section
            let formattedProposal = result.proposal || 'Failed to generate proposal';

            if (result.qa_answers && result.qa_answers.length > 0) {
                formattedProposal += '\n\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n❓ QUESTIONS & ANSWERS\n\n';
                result.qa_answers.forEach((qa, index) => {
                    formattedProposal += `Q${index + 1}: ${qa.question}\nA: ${qa.answer}\n\n`;
                });
            }

            return {
                fullProposal: formattedProposal,
                proposal: result.proposal,
                qa: result.qa_answers
            };
        }
        ...
        function copyProposal() {
            navigator.clipboard.writeText(currentProposal).then(() => {
                ...
                showToast('✅ Proposal + Q&A copied to clipboard!');
            });
        }
